...
async def _run_order(broker: BrokerClient, order: Order, grace_period_seconds: int, max_retries: int, retry_delay_seconds: float) -> None:
...
async def run_all(broker: BrokerClient, orders: list[Order], grace_period_seconds: int, max_retries: int, retry_delay_seconds: float) -> None:
    if not orders:
    ...
    async with broker:
        tasks = [
            asyncio.create_task(_run_order(broker, order, grace_period_seconds, max_retries, retry_delay_seconds))
            for order in orders
        ]
